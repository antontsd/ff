...
/*ondomready je odchytavano specificky pro ruzne browsery a na konci je window.onload, tak aby se nespustilo 2x*/
JAK.Events._windowLoadListenerId = false;
/*v nekterych prohlizecich pouzivame listener, pro jeho odveseni sem schovavam jeho id*/
...
JAK.Browser.isOld = function () {
    ...
    if (this.client == "safari" && parseFloat(this.version) < 2) {
        return true;
    }
...
JAK.query = function (query) {
    ...
    while (selectors.length) {
        ...
        /* zde udrzujeme vsechny, kteri zatim vyhovuji */
    ...
    if (action == "hide") { /* budeme schovavat */
        ...
        for (var e = 0; e < elems.length; ++e) { /* pro kazdy typ uzlu */
            ...
            for (var f = 0; f < elm.length; ++f) { /* vsechny uzly daneho typu */
                ...
                /* pokud jsou v kontejneru, pod kterym schovavame, tak fakof */
...
JAK.AbstractDecorator.prototype._$super = function () {
    ...
    var obj = null;
    /* objekt, jehoz metodu chceme volat */
    var name = null;
    /* nazev metody */
    ...
    while (i--) { /* projdu vsechny naaplikovane dekoratory */
        ...
        /* mame predchozi objekt s metodou */
...
JAK.Logger.prototype.log = function (message, sender, type) {
    ...
    if (JAK.ILogger && method == JAK.ILogger.prototype.log) {
    ...
    if (this._mode & JAK.Logger.CONSOLE) {
        this._logConsole(msg, from, t);
    }
    if (this._mode & JAK.Logger.ALERT) {
        this._logAlert(msg, from, t);
    }
    if (this._mode & JAK.Logger.URL) {
        this._logURL(msg, from, t);
    }
}
...
JAK.Logger.prototype._logConsole = function (message, sender, type) {
    ...
    switch (type) {
        case JAK.Logger.ERROR:
            console.error(str);
            break;
        case JAK.Logger.WARN:
            console.warn(str);
            break;
        case JAK.Logger.INFO:
            console.info(str);
            break;
        default:
            console.log(str);
...
JAK.Logger.prototype._logURL = function (message, sender, type) {
    ...
    var url = this._url.replace(/{([a-z]+)}/ig, function (match, what) {
        if (what in replace) {
            return encodeURIComponent(replace[what]);
        }
        return "";
    });
...
JAK.IComponents.prototype._addComponent = function (component) {
    ...
    if (typeof(component.part) == "undefined") {
        throw new Error("Invalid component named '" + component.name + "'");
    }

    var ctor = component.part;
...
JAK.IComponents.prototype.registredMethod = function (owner) {
    ...
    for (var i = 0; i < fields.length; i++) {
        ...
        for (var j in obj) {
            ...
            if (typeof(method.access) == "undefined") {
                continue;
            }
            if (method.access.indexOf('public') != 0) {
                continue;
            }
