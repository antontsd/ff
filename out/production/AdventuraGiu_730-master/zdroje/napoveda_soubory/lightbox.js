...
JAK.LightBox.prototype.$constructor = function (data, optObj) {
    ...
    for (var p in optObj) {
        if (optObj[p] instanceof Object && !optObj[p].CLASS) {
            for (var o in optObj[p]) {
                this.options[p][o] = optObj[p][o];
            }
        } else {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        /*změním výchozí fotku, vhodné jen pokud je galerie otevíraná ihned do elementu*/
        this.data.push(o);
    }
...
JAK.LightBox.prototype._addEvents = function () {
    if (this.options.handleDocumentCloseClick) {
        ...
        this.ec.push(JAK.Events.addListener(this.dom.container, 'mousedown', window, JAK.Events.stopEvent));
        /*pokud klikám do galerie, tak není vhodné zavírat okno*/
    }
...
JAK.LightBox.ImageLink.prototype._show = function (e, elm) {
    ...
    /*je třeba stopovat probublávání, protože zavření galerie se odchytává na documentu*/
...
JAK.LightBox.Transition.Fade.prototype.$constructor = function (owner) {
    ...
    this.owner = owner;
    for (var p in owner.options.transitionOpt) {
        this.options[p] = owner.options.transitionOpt[p];
    }

    this.running1 = false;
    /* běži první část animace? */
    this.running2 = false;
    /* běži druhá část animace? */

    this._secondOpacity = 0;
    /* pro případ rychlého přepínání za běhu */
    ...
    this.i2 = new JAK.Interpolator(0, 1, this.options.interval, this._step2, {
        frequency: this.options.frequency,
        endCallback: this._finish
    });
}
...
JAK.LightBox.Transition.Fade.prototype._step1 = function (value) {
    ...
    if (!this.running2 && value <= this.options.overlap) {
        this._start2();
    }
    /* už je čas nastartovat druhou část animace */
}
...
JAK.LightBox.ScaledImage.prototype.render = function () {
    ...
    this.dom.container = JAK.mel("div", null, {
        position: "absolute",
        left: "-1000px",
        top: "-1000px",
        width: "1px",
...
JAK.LightBox.StripImage.prototype.render = function (elm) {
    ...
    this.dom.tmpBox = JAK.mel('div', null, {
        position: 'absolute',
        top: '-100px',
        left: '-100px',
        width: '1px',
...
JAK.LightBox.StripImage.prototype._loaded = function (e, elm) {
    ...
    if (max > 1) {
        w = Math.floor(w / max);
        /*jelikož boz do ktereho cpu obrázky nemůže být overflow hidden kvuli ACTIVe rámečku, musím obrázky dělat radši menší*/
        h = Math.floor(h / max);
        if (w && h) {
